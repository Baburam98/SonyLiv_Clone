...
function Footer() {
    ...
    const footer = footerData.map(item=>{
        return(
            <p key={item.id}><strong>{item.title}</strong><span style={{color:"rgb(165, 164, 164)"}}>{item.Data}</span></p> 
            // <div className="footer" key={item.id}>
            //     <p><strong>{item.title}</strong><span style={{color:"rgb(165, 164, 164)"}}>{item.Data}</span></p> 
            // </div>
        )
    })
    ...
    return (
        <div>
            ...
            <p id="copy">&copy;  Sony Pictures Networks India Pvt.Ltd</p>
        </div>
    )
}
